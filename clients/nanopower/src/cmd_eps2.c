...
int cmd_eps_config3_edit(struct command_context *ctx)
{
	...
	printf("Press <.> to skip\r\n");
	for (int i = 0; i < 8; i++) {
		printf("CUR_LIM[%u] (0 - 2500 mA, 0 to disable)\r\n", i);
	...
	for (int i = 0; i < 8; i++) {
		if (nanopower_config3.cur_lim[i] > 2500) {
			printf("\r\nWoops, you  have set current limit[%d] > 2500 mA, please change this!\r\n", i);
		}
	}
	if ((nanopower_config3.cur_ema_gain < 10) || (nanopower_config3.cur_ema_gain > 90)) {
		printf("\r\nWoops, you  have set current EMA gain outside range [10 - 90], please change this!\r\n");
	}
	conf3_valid = 1;
