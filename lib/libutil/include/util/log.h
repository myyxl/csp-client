...
#define log_trace_group(...) do {} while(0);
...
#define LOG_GROUP_EXTERN(...)

#elif defined(LOG_PRINTF)
#define log_trace(format, ...) { 	do { if (format != NULL) {printf(format, ##__VA_ARGS__); } printf("\r\n"); } while(0); }
...
#define log_trace(format, ...) { 	do { if (format != NULL) { COLOR_PRINTF(COLOR_WHITE, format, ##__VA_ARGS__); } printf("\r\n"); } while(0); }
