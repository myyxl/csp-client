...
int watch_handler(struct command_context * context) {
	...
	while(1) {
		...
		if (command_run(new_command) == CMD_ERROR_NOTFOUND)
...
command_t __root_command cmd_dfl[] = {
	{
		...
		.name = "sleep",
		.help = "gosh: Sleep X ms",
		.usage = "<time>",
		.handler = sleep_handler,
	},{
		.name = "watch",
		.help = "gosh: Run cmd at intervals, abort with key",
		.usage = "<n> <command>",
		.handler = watch_handler,
	},{
		.name = "batch",
		.help = "gosh: Run multiple commands",
...
#ifndef __linux__
	{
		...
		.name = "ps",
		.help = "gosh: List tasks",
		.handler = ps_handler,
	},{
		.name = "peek",
		.help = "gosh: Dump memory",
		.usage = "<addr> <len>",
		.handler = peek_handler,
	},{
		.name = "poke",
		.help = "gosh: Change memory",
		.usage = "<addr> <value>",
